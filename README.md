...
顔が見えないリモートワークにおいて、他の同僚の忙しさは気になりますよね。分かる人ならば誰でもいいから答えて欲しいという質問を会社の同僚にメールで送りつけるのは気がひけます。一方で、リモートワークでよく使われているチャットや社内SNSで質問しても誰からも反応なし、という経験はありませんか？.Chain()はあなたの質問の宛先をコンピュータが自動的に決めてくれます。しかも、高確率で誰かから回答をもらえるしくみを搭載しています。
...
| .Chain()を使えば、質問相手を決めるのに頭を悩ませていませることがありません。質問作成時に宛先の部署を指定すれば、コンピュータがユーザのプロフィール情報をもとに自動的に宛先を決定してくれます。| 質問を受け取った人は、一定時間内に返信するかパスするかを選択できます。忙しくて質問をチェックできない場合も、自動パスになるので安心です。たとえパスしても、あなたの名前が質問者に分かることはありません。 |
...
| 質問者は、返事が返ってこないという心配をする必要がありません。なぜならば、.Chain()はAI機能を搭載しており、自動的に返信してくれるからです。このAIの回答から、疑問解決のヒントを探してみてください。| 質問・回答を行ってゆくうちに、ユーザ同士のつながりが作成されます。これにより、自分と他の人とのやりとりの頻度や、どのような興味・スキルでつながっているかが図で一目でわかります。 |
...
* ローカルで起動  
3つのターミナルを開き、以下のコマンドをそれぞれのターミナルに入力し、起動してください。
